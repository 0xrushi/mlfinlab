...
class HistoryWeightRegression:
    ...
    def __init__(self, Y_train: np.array, X_train: np.array, check_condi_num: bool = False):
        ...
        self.check_condi_num = check_condi_num
        self.x_avg = np.mean(X_train, axis=0)
        self.y_avg = np.mean(Y_train)
        self.cov_mtx, self.fisher_info_mtx = self._calc_cov_and_fisher(X_train, check_condi_num)
    ...
    def get_fit_result(self) -> dict:
        ...
        # This method should return a dictionary with fit results and statistics.
        # Here, we return a placeholder dictionary. You would need to add appropriate calculations for each statistic.
        return {"cov_mtx": self.cov_mtx, "fisher_info_mtx": self.fisher_info_mtx}
    ...
    def predict(self, X_t: np.array, relev_ratio_threshold: float = 1) -> np.array:
        ...
        """
        results = np.array([self.predict_one_val(x_t, relev_ratio_threshold) for x_t in X_t])
        return results
    ...
    def predict_one_val(self, x_t: np.array, relev_ratio_threshold: float = 1) -> Tuple[float, float]:
        ...
        """
        X_sub, Y_sub, _, std = self.find_subsample(x_t, relev_ratio_threshold)
        y_avg_sub = np.mean(Y_sub)
        y_t = y_avg_sub + 1/(len(X_sub)-1) * (x_t - self.x_avg).T @ self.fisher_info_mtx @ (X_sub - self.x_avg).T @ (Y_sub - y_avg_sub)
        return y_t, std
    ...
    def find_subsample(self, x_t: np.array, relev_ratio_threshold: float = 1, above: bool = True) -> Tuple[np.array, np.array, np.array, float]:
        ...
        """
        relevances = np.array([self.calc_relevance(x_t, x_i) for x_i in self.X_train])
        threshold = np.percentile(relevances, relev_ratio_threshold * 100 if above else 100 - relev_ratio_threshold * 100)
        indices = np.where(relevances >= threshold) if above else np.where(relevances < threshold)
        X_sub = self.X_train[indices]
        Y_sub = self.Y_train[indices]
        var_y = np.var(Y_sub)
        y_mean = np.mean(Y_sub)
        var_r = np.var(relevances[indices])
        n = len(Y_sub)
        std = np.sqrt(((n-1)/n**2 * var_y) + (1/n * y_mean**2) + (var_y/n + y_mean**2/(n-1))*var_r)
        return X_sub, Y_sub, indices, std
    ...
    def _calc_cov_and_fisher(X: np.array, check_condi_num: bool = False) -> Tuple[np.array, np.array]:
        ...
        if check_condi_num:
            condi_num = np.linalg.cond(cov_mtx)
            if condi_num > 1e10:
                warnings.warn(f"The condition number is very large: {condi_num}")
    ...
    def calc_relevance(self, x_i: np.array, x_j: np.array, fisher_info_mtx: np.array = None) -> float:
        ...
        return self.calc_sim(x_i, x_j, fisher_info_mtx) + self.calc_info(x_i, fisher_info_mtx) + self.calc_info(x_j, fisher_info_mtx)
    ...
    def calc_sim(self, x_i: np.array, x_j: np.array, fisher_info_mtx: np.array = None) -> float:
        ...
        if fisher_info_mtx is None:
            fisher_info_mtx = self.fisher_info_mtx
        return -0.5 * (x_i - x_j).T @ fisher_info_mtx @ (x_i - x_j)
    ...
    def calc_info(self, x_i: np.array, fisher_info_mtx: np.array = None) -> float:
        ...
        if fisher_info_mtx is None:
            fisher_info_mtx = self.fisher_info_mtx
        return 0.5 * (x_i - self.x_avg).T @ fisher_info_mtx @ (x_i - self.x_avg)
